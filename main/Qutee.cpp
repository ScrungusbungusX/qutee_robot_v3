...
void Qutee::init(){
  ...
  // Wire.setPins(GPIO_NUM_17,GPIO_NUM_18);
  // uart has to use 17 and 18 in QuteeDxlPortHandler.cpp
  //Wire.begin();

  //init_tft(); // TEMPORARY FIX TEMPORARY FIX DONT FORGET
  ...
  //tft_load_screen();      // ALSO TEMP FIX DONT FORGET
  ...
  //init_imu(); // TEMPORARY FIX TEMPORARY FIX DONT FORGET
...
bool Qutee::init_imu(){
  ...
  //Wire.setPins(GPIO_NUM_42,GPIO_NUM_41);
  //Wire.begin();
  ...
  if (!this->_bno.begin()) {
      ESP_LOGE("IMU","No BNO055 detected");
      return false; // STOP USING WHILE(1) IT UPSETS WATCHDOG
  }
...
esp_err_t Qutee::init_NVS() {
    ESP_LOGI("TAG", "Init NVS Start...");
...
void Qutee::send_actions(const Actions_t& actions){
  ...
  if(this->_dxl.syncWrite(&this->_sw_infos) == true){
    // why was all this commented out?
    ESP_LOGI("Control Loop","[SyncWrite] Success");
  } else {
    ESP_LOGE("Control Loop","[SyncWrite] Fail, Lib error code: %ld ",this->_dxl.getLastLibErrCode()); // TODO fix this error
  }
...
// REPLACEMENT SEE ABOVE - also use tab for spacing like normal people
void Qutee::run_episode() {
    // Fill states and actions for the entire episode
    for (size_t step = 0; step < NB_STEPS; ++step) {
        // Get current robot state from motors & sensors
        get_state(_state);

        // Compute action using policy
        // Actions_t action = _policy(_state);

        Actions_t action;
        _policy.pos(_state, action);
        ...
        // Optional: update TFT screen every few steps
        //if (step % 10 == 0) {
            //tft_update_data_screen(_state, action, CONFIG_CONTROL_FREQUENCY);
        //}

        // Wait for next control step
        // PERIOD is defined in microseconds
        //esp_rom_delay_us(PERIOD);
        // replacing this ^ might upset watchdog otherwise
