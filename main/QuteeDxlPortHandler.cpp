...
{
    baud_ = baud;
    ESP_LOGI(this->TAG, "START CUSTOM PORT HANDLER");
...
//    UART1.rs485_conf.rs485tx_rx_en = 1;  // loopback (1), so collision detection works
    ...
    setOpenState(true);
    ESP_LOGI(this->TAG, "START CUSTOM PORT HANDLER: DONE");
}
...
{
    uint8_t byte;
    int len = uart_read_bytes(uart_num_, &byte, 1, 0); // non-blocking

    if (len == 1)           // just gonna overhaul this section because its crap
        return byte;
...
size_t QuteeDxlPortHandler::write(uint8_t c) 
{
    ...
    // ESP_ERROR_CHECK(uart_flush(uart_num_)); // Aedan here, isnt this just atrocious for performance?
